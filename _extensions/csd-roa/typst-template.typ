...
#let report-template(
  ...
  // customizing the cover page of the ROA
  {
    ...
    grid(
      ...
      [SRM(s):], if srms != none { srms.join(", ") } else [NA],
      ...
      [Agreements:], if agreements != none { agreements.join(", ") } else [NA],
      [Approvals:], if approvals != none { approvals.join(", ") } else [NA],
