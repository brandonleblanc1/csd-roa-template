...
#let report-template(
  ...
  // customizing the cover page of the ROA
  {
    ...
    grid(
      ...
      [SRM(s):], if srms != none { srms.join(", ").replace("\\", "") } else [NA],
      ...
      [Constituent(s):], if constituents != none { constituents.join(", ") } else [NA],
      [Purpose:], purpose,
      [Control(s):], controls.join(", ").replace("\\", ""),
      [Date(s) of Lab Work:], lab-dates,
      [Collaborator(s):], if collaborators != none { collaborators.join(", ") } else [NA],
      [Quality Documents:], quality-docs.join(", "),
      [Agreements:], if agreements != none { agreements.join(", ").replace("\\", "") } else [NA],
      [Approvals:], if approvals != none { approvals.join(", ").replace("\\", "") } else [NA],
